...
- **Шрифт**: Для корректного рендеринга текста всегда указывайте пару свойств: `-unity-font` и `-unity-font-definition`.
  - **Russo One** `var(--font-russo-one)`: "Голос игры" — для кнопок, экшенов, призывов к действию ("ОТКРЫВАЙ ЯЩИК").
  - **JetBrains Mono** `var(--font-jetbrains-mono)`: "Бухгалтер" — для строгого, моноширинного вывода данных (деньги, цены, статистика, время).
