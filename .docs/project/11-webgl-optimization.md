...
### Шрифты
- Используй **максимум два** шрифта (в нашем случае: Russo One и JetBrains Mono).
- Подключай через `TextMeshPro` / UI Toolkit — он кэширует глифы и работает быстрее стандартного `Text`.
