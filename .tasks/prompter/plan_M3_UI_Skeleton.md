...
### Task M3.3.2 — Точечная сборка MarketPanel (Гача/Множители) ❌
**Цель:** Воспроизвести интерфейс покупки лотов и мультипликаторов.
- Метрики LotSelectionMenu: ширина 350px, gap 12, padding 16.
- Реализовать эффект свечения (`$glow_cyan` / `box-shadow`) и скругление углов 12px.
- Добавить центральный блок-кнопку `OpenLotAction` и блок `PriceIndicator`.
> **Промпт для Промптера:** Напиши ТЗ для Воркера на верстку `MarketPanel.uxml` и `Roulette.uss`. Максимальное внимание на тени (shadowType outer) и полупрозрачные фоны.
