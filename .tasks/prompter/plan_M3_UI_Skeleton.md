...
### Task M3.3.1 — Точечная сборка HUDPanel (TopBar) ❌
**Цель:** Сверстать шапку 1 к 1 с макетом.
- Использовать метрики: высота 137px, padding 32x0, gap 16.
- Применить шрифты (Inter/Orbitron) и градиенты.
- Реализовать три логических зоны: Баланс (слева), Токены/Энергия (по центру), Профиль (справа).
> **Промпт для Промптера:** Напиши ТЗ для Воркера на создание точной верстки `HUDPanel.uxml` по макету Pencil, обнови стили в `HUD.uss`. Пока без скриптов логики, только UI Toolkit Visual Elements.
